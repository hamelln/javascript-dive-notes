...
AST는 바로 기계어로 번역하기 전에 Bytecode로 번역합니다. 기계어보다 메모리 차지율이 적고 JS보다 번역이 빠르기 때문에 성능과 최적화를 하기 적합하기 때문입니다. 이렇게 번역한 Bytecode는 실행시 직렬화해서 `Heap`에 보관했다가 동일한 스크립트를 만나면 역직렬화해서 가져온 다음 실행합니다. 참고로 바이트코드 생성 중에 클로저 로직 준비를 같이 합니다.
